...
const EmployeePartiallyCompletedTasks = () => {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        const api = `${import.meta.env.VITE_BACKEND_URL}/employee/showtask?id=${localStorage.getItem(
          "empid"
        )}`;
        const res = await axios.get(api);

        // ✅ Filter partial tasks
        const partialTasks = (res.data.tasks || []).filter(
          (t) => t.taskstatus === "partial"
        );
    ...
    >
      ...
      >
        {loading ? (
          ...
          >
            No partially completed tasks found.
          </p>
        ) : (
          <Table
            bordered
            ...
            <tbody>
              {tasks.map((task, index) => (
                ...
                >
                  ...
                  <td
                    style={{
                      color: "#f59e0b", // Yellow for partial
                      fontWeight: 700,
                    }}
                  >
                    🟡 Partial
...
export default EmployeePartiallyCompletedTasks;

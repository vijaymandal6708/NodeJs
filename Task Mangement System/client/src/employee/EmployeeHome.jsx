  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:9002/employee/homeshowtask?id=${empid}`);

        // Backend returns: { total, completed, partial, pending, notStarted }
        const d = res.data;

        // Make chart-friendly arrays
        setTaskStats([
          { name: "Completed", value: d.completed },
          { name: "Partial", value: d.partial },
          { name: "Pending", value: d.pending },
          { name: "Not Started", value: d.notStarted },
        ]);

        // Employees count — optional for employee side
        setEmployeeStats([{ name: "Your Tasks", value: d.total }]);
      } catch (err) {
